...
{{- $modelName := required "Please specify a valid reranking_model_name name in your Helm chart values" .Values.reranking_model_name }}
{{- $port := "8109" }}

apiVersion: v1
kind: ConfigMap
metadata:
  name: vllm-reranking-config
  labels:
        ...
        - name: vllm-reranking
          ...
          command:
            ...
            - |
                ...
                {{- end }}
                python3 -m vllm.entrypoints.openai.api_server --model {{ $modelName }} --dtype $VLLM_DTYPE --download-dir /data --host 0.0.0.0 --port {{ $port }}
          resources:
...
kind: Deployment
metadata:
  name: vllm-reranking-svc-deployment
  labels:
    {{- include "manifest.labels" (list .filename .) | nindent 4 }}
spec:
...
spec:
  ...
  - type: Object
    object:
      metric:
        name: vllm_reranking_request_queue_time
      describedObject:
        apiVersion: v1
        kind: Service
        name: vllm-reranking-svc
      target:
        type: Value
