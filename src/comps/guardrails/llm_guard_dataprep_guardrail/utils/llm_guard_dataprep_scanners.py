...
class DataprepScannersConfig:

    def __init__(self, config_dict):
        self._dataprep_scanners_config = {
    ...
    def _create_dataprep_scanner(self, scanner_name, scanner_config):
        ...
        if scanner_name == 'ban_substrings':
            ...
            return self._create_secrets_scanner()
    ...
    def create_enabled_dataprep_scanners(self):
        ...
        """
        enabled_scanners_names_and_configs = {k: v for k, v in self._dataprep_scanners_config.items() if v.get("enabled")}
        ...
        for scanner_name, scanner_config in enabled_scanners_names_and_configs.items():
            try:
                ...
                scanner_object = self._create_dataprep_scanner(scanner_name, scanner_config)
                enabled_scanners_objects.append(scanner_object)
            except ValueError as e:
                err_msg = f"A ValueError occured during creating dataprep scanner {scanner_name}: {e}"
                logger.error(err_msg)
                err_msgs[scanner_name] = err_msg
                self._dataprep_scanners_config[scanner_name]["enabled"] = False
                continue
            except TypeError as e:
                err_msg = f"A TypeError occured during creating dataprep scanner {scanner_name}: {e}"
                logger.error(err_msg)
                err_msgs[scanner_name] = err_msg
                self._dataprep_scanners_config[scanner_name]["enabled"] = False
                continue
            except Exception as e:
                err_msg = f"An unexpected error occured during creating dataprep scanner {scanner_name}: {e}"
                logger.error(err_msg)
                err_msgs[scanner_name] = err_msg
    ...
    def changed(self, new_scanners_config):
        ...
        previously_enabled_scanners = {k: v for k, v in self._dataprep_scanners_config.items() if v.get("enabled")}
