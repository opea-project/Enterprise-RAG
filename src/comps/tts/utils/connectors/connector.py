...
class TTSConnector(ABC):
    ...
    async def _validate(self) -> None:
        """
        Validates the TTS model by synthesizing speech from a test query.

        Raises:
            RuntimeError: If there is an error initializing the TTS model.
        """
        try:
            await self.synthesize_speech("test")
            logger.info("TTS model server validated successfully.")
        except RuntimeError as e:
            logger.exception(f"Error initializing the TTS model: {e}")
            raise
        except Exception as e:
