...
class SharepointHelper():
    ...
    def list_site_files(self, site_name: str) -> list:
        ...
        # Get items from the default Drive (Document Library)
    ...
    def upload_file_to_site(self, site_name: str, file_path: str) -> dict:
        ...
        # Upload file to the root drive
        file_name = os.path.basename(file_path)
        upload_url = f"{GRAPH_API_BASE}/sites/{site_id}/drive/root:/{file_name}:/content"
        ...
        if upload_resp.status_code not in (200, 201):
            logger.error(f"Failed to upload file {file_name} to site {site_name}: {upload_resp.text}")
            upload_resp.raise_for_status()

        logger.info(f"File {file_name} uploaded successfully to site {site_name}")
