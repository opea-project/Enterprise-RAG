...
def test_json_text_only(dataprep_api_helper, chatqa_api_helper):
    """*.json file learning capabilities (pure text inside the file)"""
    question = "What is RagRag?"
    response = upload_and_ask_question(dataprep_api_helper, chatqa_api_helper, "story.json", question)
    assert "whale" in response.lower(), UNRELATED_RESPONSE_MSG


@allure.testcase("IEASG-T105")
def test_doc_text_only(dataprep_api_helper, chatqa_api_helper):
