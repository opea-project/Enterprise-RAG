...
def test_singleton_behavior(mock_langchain):
    instance1 = OPEAEmbedding("model1", "model_server", "langchain")
    instance2 = OPEAEmbedding("model1", "model_server", "langchain")

    assert instance1 is instance2

    OPEAEmbedding._instance = None

def test_singleton_behavior_wrong_framework(mock_langchain):
    OPEAEmbedding("model1", "model_server", "langchain")
    with pytest.raises(Exception) as exc_info:
        OPEAEmbedding("model2", "model_server", "llama_index")
...
def test_initialization(mock_llamaindex):
    model_name = "test_model"
    model_server = "test_server"
    framework = "llama_index"
    endpoint = "http://test-endpoint"
    embedding = OPEAEmbedding(model_name, model_server, framework, endpoint)

    assert embedding._model_name == model_name
    assert embedding._model_server == model_server
    assert embedding._framework == framework
...
def test_import_langchain(mock_langchain):
    model_name = "test_model"
...
def test_import_not_found_langchain():
    model_name = "test_model"
    model_server = "model_server"
    framework = "langchain"
    endpoint = "http://test-endpoint"
    ...
    with pytest.raises(Exception) as exc_info:
        OPEAEmbedding(model_name, model_server, framework, endpoint)
        assert "langchain module not found" in exc_info.value.args[0]

    OPEAEmbedding._instance = None

def test_import_llamaindex(mock_llamaindex):
    model_name = "test_model"
    model_server = "test_server"
    framework = "llama_index"
    endpoint = "http://test-endpoint"
...
def test_import_not_found_llamaindex():
    model_name = "test_model"
    model_server = "model_server"
    framework = "llama_index"
    endpoint = "http://test-endpoint"
