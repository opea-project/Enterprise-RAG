...
export interface UseGuardServiceCardOptions {
  changeArguments: ChangeArgumentsFunction;
}

export const useGuardServiceCard = <T>(
  guardName: string,
  args: T | undefined,
  options: UseGuardServiceCardOptions,
) => {
  const { changeArguments } = options;
  ...
  const onConfirmChangesButtonClick = () => {
    const changeArgumentsRequest = [
      {
        name: guardName,
        data: argumentsForm as Record<string, ServiceArgumentInputValue>,
      },
    ];
